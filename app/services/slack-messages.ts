...
export default class SlackMessagesService extends Service {
  ...
  async fetchMessages(channelId: string): Promise<void> {
    ...
    try {
      ...
      const body = `token=${this.slackAuth.token}&channel=${encodeURIComponent(channelId)}&limit=28&ignore_replies=true&include_pin_count=true&inclusive=true&no_user_profile=true&include_stories=true&include_free_team_extra_messages=true&include_date_joined=false&include_all_metadata=true`;
      ...
      console.log('[SlackMessages] Response status:', response.status);
  ...
  private async fetchAllUsers(): Promise<void> {
    ...
    try {
      ...
      const response = await fetch('https://ibm.enterprise.slack.com/api/users.list', {
        method: 'POST',
        headers: {
          'Cookie': `d=${this.slackAuth.dCookie}`,
          'Content-Type': 'application/x-www-form-urlencoded',
        },
        body: `token=${encodeURIComponent(this.slackAuth.token)}&limit=1000`,
      });

      console.log('[SlackMessages] Users list response status:', response.status);
      const data = await response.json();
  ...
  async sendMessage(text: string): Promise<void> {
    ...
    try {
      ...
      const response = await fetch('https://ibm.enterprise.slack.com/api/chat.postMessage', {
        method: 'POST',
        headers: {
          'Cookie': `d=${this.slackAuth.dCookie}`,
          'Content-Type': 'application/x-www-form-urlencoded',
        },
        body: `token=${encodeURIComponent(this.slackAuth.token)}&channel=${encodeURIComponent(this.currentChannelId)}&text=${encodeURIComponent(text)}`,
      });

      console.log('[SlackMessages] Send message response status:', response.status);
      const data = await response.json();
